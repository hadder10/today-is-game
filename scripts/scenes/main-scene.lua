...
local round = 0
local headlines = {} -- Empty table for headlines
...
function main_game.load()
    ...
    menu_button = {
        ...
        x = 55,      -- немного правее от края main_news
        y = 150,     -- немного ниже от верха main_news
        width = 50,  -- установите реальную ширину вашей иконки
        height = 50, -- установите реальную высоту вашей иконки
        scale = 1.0
    }
    ...
    button_1 = {
        x = 70,      -- Сдвигаем левее
        y = 800,
        width = 180, -- Увеличиваем ширину
        height = 100,
        visible = false,
        scale = 1.0,
        text = "Reset" -- Добавляем текст
    }

    button_2 = {
        x = win_width - 250, -- Сдвигаем правее
        y = 800,
        width = 180,         -- Увеличиваем ширину
        height = 100,
        visible = false,
        scale = 1.0,
        text = "Next day" -- Добавляем текст
    }
    ...
    -- В love.load() добавим параметры для overlay:
    menu_overlay = {
        visible = false,
        alpha = 0.7,              -- прозрачность
        color = { 0.1, 0.1, 0.1 } -- тёмно-серый цвет
    }

    -- В функции main_game.load() добавим параметры для кнопки выхода в меню:
    menu_exit_button = {
        text = "Exit to menu",
        width = 200,
        height = 60,
        x = win_width / 2 - 100, -- центрируем по горизонтали
        y = win_height / 2 - 30, -- центрируем по вертикали
        scale = 1.0
    }

    -- Generate initial headlines and descriptions for each faction
    ...
    -- Добавляем инициализацию info_window
    info_window = {
        x = win_width / 2 - 200, -- центрируем окно
        y = win_height / 2 - 150,
        width = 400,             -- размеры окна
        height = 300,
        visible = false,         -- изначально окно скрыто
        text = ""                -- текст для отображения
    }

    -- Добавляем структуру для оповещения о проигрыше
    game_over_popup = {
        visible = false,
        x = win_width / 2 - 300,
        y = win_height / 2 - 200,
...
function drawSomeTiles(x, y, object_2_draw, icon)
    if icon.visible then
        -- Вычисляем смещение для центрирования при масштабировании
        local offsetX = (icon.scale - 1) * 100 -- половина ширины (200/2)
        local offsetY = (icon.scale - 1) * 25  -- половина высоты (50/2)

        -- Рисуем фоновый прямоугольник с учетом масштаба
        ...
        -- Рисуем иконку по центру с учетом масштаба
        ...
        -- Добавляем "+" или "-" с учетом масштаба
        love.graphics.setColor(0, 0, 0)
...
function main_game.update(dt)
    ...
    -- Обновляем масштаб для переносимых объектов
    ...
    -- Проверяем наведение на кнопку меню
    ...
    -- Если оверлей активен, проверяем наведение на кнопку выхода в меню
...
function main_game.draw()
    ...
    -- Отрисовка menu_button с учетом масштаба
    ...
    -- В конце функции love.draw() добавим отрисовку overlay:
    -- Отрисовка menu overlay
    if menu_overlay.visible then
    ...
    -- Отрисовка текстов на draggable_cal только если чекбокс заполнен
    love.graphics.setColor(0, 0, 0)
    ...
    if info_window.visible then
        ...
        -- Возвращаем цвет в нормальное состояние после отрисовки окна
        love.graphics.setColor(1, 1, 1)
    end

    -- Отрисовка окна окончания игры, если активно
...
function main_game.mousepressed(x, y, button, istouch, presses)
    ...
    -- Получаем позицию мыши в начале функции
    local mx, my = love.mouse.getPosition()
    ...
    if button == 1 then
        ...
        -- Проверяем клик по кнопке выхода в меню
        if menu_overlay.visible and
            ...
            _G.switchScene("menu")       -- переключаем сцену на меню
            menu_overlay.visible = false -- скрываем оверлей
            return
        end

        -- Если overlay видим и кликнули в любое место - скрываем его
        if menu_overlay.visible then
        ...
        if checkAllBoxes() then
            ...
            elseif x > button_2.x and x < button_2.x + button_2.width and
                ...
                if checkGameOver() then
                    -- Делаем все элементы управления неактивными
                ...
                if checkbox_4.checked and checkbox_4.stored_icon then
                    ...
                    -- ОЧЕНЬ ВАЖНО ДЛЯ ГЕНЕРАЦИИ
                    round = round + 1

                    -- Генерируем уникальный текст для каждой фракции
                    local factions_data = {
                    ...
                    for _, data in ipairs(factions_data) do
                        local prompt = {
                            faction = data.faction -- передаем только faction
                        }
                        ...
                        if headline and description then
                            ...
                            if headline ~= "" then
                                -- Обновляем тексты для конкретной иконки
                                news_texts[data.icon] = headline
                    ...
                    -- Добавляем в историю заголовок от доминирующей фракции
                    -- local main_prompt = build_prompt()
                    -- local headline, _ = gen.generate(main_prompt)
                    -- if headline then
        ...
        if checkbox_1.checked and x > pivot.x - checkbox_1.x and x < pivot.x - checkbox_1.x + checkbox_1.width and
            ...
            icon_1.visible = true
            checkbox_1.checked = false
            checkbox_1.stored_icon = nil -- очищаем stored_icon
            object_1.current_text = ""   -- очищаем текст
        ...
        elseif checkbox_2.checked and x > pivot.x + checkbox_2.x and x < pivot.x + checkbox_2.x + checkbox_2.width and
            ...
            icon_2.visible = true
            checkbox_2.checked = false
            checkbox_2.stored_icon = nil -- очищаем stored_icon
            object_2.current_text = ""   -- очищаем текст
        ...
        elseif checkbox_3.checked and x > pivot.x + checkbox_3.x and x < pivot.x + checkbox_3.x + checkbox_3.width and
            ...
            icon_3.visible = true
            checkbox_3.checked = false
            checkbox_3.stored_icon = nil -- очищаем stored_icon
            object_3.current_text = ""   -- очищаем текст
        ...
        elseif checkbox_4.checked and x > pivot.x - checkbox_4.x and x < pivot.x - checkbox_4.x + checkbox_4.width and
            ...
            icon_4.visible = true
            checkbox_4.checked = false
            checkbox_4.stored_icon = nil -- очищаем stored_icon
            object_4.current_text = ""   -- очищаем текст
